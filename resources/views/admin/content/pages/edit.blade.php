<form method="POST" action="{{ route('admin.content.pages.edit', ['id' => $data['page']['id']]) }}">
	{{ csrf_field() }}
	...
	<div class="lg-9 md-8 sm-12 column column__first">
		...
		<input type="text" class="form__input" name="title" value="{{ $data['page']['title'] }}">
		...
		<div class="form__group">
			...
			<input class="form__group__input form__input" name="slug" value="{{ $data['page']['slug'] }}">
		</div>

		<!-- Content -->
		<label class="form__label" for="content">{{ static_text('form.content.title') }}</label>
		<textarea class="form__input form__input--textarea" name="content" id="tinyLarge">{!! $data['page']['content'] !!}</textarea>

		{{ Hook::run('page_content') }}
